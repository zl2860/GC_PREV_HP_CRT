...
m1<-coxph(Surv(Surv_case_GC_20221231,Y_GC_20221231)~A5B5C,data=dff)
summary(m1)

# Prior locked database
table(dat$Y_GC_20221231_old)

## figure
p1<-ggsurvplot(survfit(Surv(Surv_case_GC_20221231_old/365.25,Y_GC_20221231_old)~try1,data=dat),
               fun="event", 
               risk.table=TRUE,
               xlim=c(0,12),xlab="Follow-up (year)",
               censor.size=3,
...
doc <- read_pptx()#readOffice
doc <- add_slide(doc, layout = "Title and Content", master = "Office Theme")
doc <- ph_with(doc, my_vec_graph, location = ph_location_fullsize() )
print(doc, target = "F2_prior.pptx",width=5)

## HR
dff<-dat%>% filter(A5B5C=='Group B' | A5B5C=='Group A')
m1<-coxph(Surv(Surv_case_GC_20221231_old,Y_GC_20221231_old)~A5B5C,data=dff)
summary(m1)
